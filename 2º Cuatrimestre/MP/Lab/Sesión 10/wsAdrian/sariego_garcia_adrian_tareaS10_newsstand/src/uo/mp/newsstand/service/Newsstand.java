...
public class Newsstand {
	 ...
	 * @throws AlreadyExistException if the product's name is repeated
	 */
	public void addPublication(Publication p) throws AlreadyExistException {
		...
		if (searchByName(p.getName()) != -1) {			
			...
			// throw new RepeatedException("La publicación está repetida"); Esta la tenia hecha yo
			// throw new NewsStandException("La publicación " + p.getName + " ya existe"); // otra de ejemplo
		}
		publications.add(p);
	}
	 ...
	 * @throws LogicalException the product does not exist
	 */
	public void removePublication(String name) throws LogicalException {
		int pos = searchByName(name);
		if (pos == -1) {
			throw new LogicalException("La publicación a eliminar no existe");
		}
	...
	public List<Publication> getPublications() {
		...
		// Si le paso publications, lo que esta haciendo es hacer una copia
		// (es un constructor de las colecciones de java) y lo tengo que
		// hacer en mis listas
	...
	public void createOrders() {
		for (Publication publication : publications) {
			if (publication.getStock() < 10) {
				//orders.add(new Order(publication.getName(), publication.getNumberOfPublicationsNeeded()));
	...
	//No me hace falta
//	private boolean isRepeated(Publication p) {
//		for (Publication publication : publications) {
//			if (publication.equals(p))
//				return true;
//		}
//		return false;
//	}
