...
public class WordAnalyzer {
	...
	public WordAnalyzer(String aWord) {
		...
		ArgumentsCheck.isTrue(aWord.trim().length() > 0, "Cadena vacía");// COMPROBACIÓN CADENA VACÍA
		word = aWord;
	}
	...
	public int countGroupsOfRepeatedCharacters() {
		...
		boolean next = false;
		for (int i = 0; i < word.length() - 1; i++) {
			if (word.charAt(i) == word.charAt(i + 1) && next == false) {
				count++;
				next = !next;
			} 
			if(word.charAt(i) != word.charAt(i + 1) && next == true) {
				next = false;
			}
